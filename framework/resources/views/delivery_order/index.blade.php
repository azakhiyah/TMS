...
@endsection
@section("breadcrumb")
<li class="breadcrumb-item active">@lang('fleet.do')</li>
@endsection
@section('content')
<div class="row">
  <div class="col-md-12">
    ...
    <div class="card card-info">
      <div class="card-header">
        <h3 class="card-title">@lang('fleet.do') &nbsp;<a href="{{ route('delivery_order.create')}}" class="btn btn-success">@lang('fleet.do_add')</a>
        ...
        <table class="table" id="data_table" style="padding-bottom: 25px">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{$row->no_PO}}</td>
              <td>{{$row->no_LO}}</td>
              <td>@if($row->customers){{$row->customers->name}}@endif</td>
              <td>@if($row->customerslocation){{$row->customerslocation->address}}@endif</td>
              <td>@if($row->depo){{$row->depo->name}}@endif</td>
              <td>@if($row->product){{$row->product->name}}@endif</td>
              <td>{{$row->qty_delivery}}</td>
              ...
              <a class="dropdown-item" href="{{url("admin/delivery_order/".$row->id."/edit") }}"> <span aria-hidden="true" class="fa fa-edit" style="color: #f0ad4e;"></span> @lang('fleet.edit')</a>
              ...
              <a class="dropdown-item openBtn" data-id="{{$row->id}}" data-toggle="modal" data-target="#myModal2" id="openBtn">
                    <span class="fa fa-eye" aria-hidden="true" style="color: #398439"></span> @lang('fleet.do_view')
                    </a>
              </div>
              </div>
              {!! Form::open(['url' => 'admin/delivery_order/'.$row->id,'method'=>'DELETE','class'=>'form-horizontal','id'=>'form_'.$row->id]) !!}
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        {!! Form::open(['url'=>'admin/delete-delivery_order','method'=>'POST','id'=>'form_delete']) !!}
...
<div id="myModal2" class="modal fade" role="dialog" tabindex="-1">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">@lang('fleet.do')</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body">
...
<script type="text/javascript">
  ...
  $('.openBtn').click(function(){
    ...
    $('#myModal2 .modal-body').load('{{ url("admin/delivery_order/event")}}/'+id,function(result){
