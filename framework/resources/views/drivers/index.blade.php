  ...
  <div class="col-md-12">
    ...
    <div class="card card-info">
      <div class="card-header">
        ...
        @lang('fleet.drivers')
        &nbsp;
        <a href="{{ route('drivers.create')}}" class="btn btn-success">@lang('fleet.drivers_add')</a>
        ...
        <table class="table" id="data_table">
          <thead class="thead-inverse">
            <tr>
              ...
              <th>@lang('fleet.drivers_name')</th>
							<th>@lang('fleet.drivers_address')</th>
							<th>@lang('fleet.drivers_phone')</th>
							<th>@lang('fleet.drivers_sex')</th>
							<th>@lang('fleet.drivers_sim_number')</th>,
							<th>@lang('fleet.drivers_sim_exp_date')</th>
							<th>@lang('fleet.drivers_image')</th>
							<th>@lang('fleet.drivers_emergency_contact')</th>
              <th>@lang('fleet.drivers_gatepass')</th>
              <!-- <th>@lang('fleet.drivers_gatepass_rls_date')</th> -->
              <th>@lang('fleet.drivers_gatepass_exp_date')</th>
							<th>@lang('fleet.drivers_join_date')</th>
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td>{{$row->name}}</td>
						  ...
						  <td>{{$row->sim_exp_date}}</td>
              <!-- <td>
              @if($row->driver_image != null)
              <img src="{{asset('uploads/'.$row->icon)}}" height="70px" width="70px">
              @else
              <img src="{{ asset("assets/images/truck.PNG")}}" height="70px" width="70px">
              @endif
              </td> -->
						  <td>{{$row->driver_image}}</td>
						  <td>{{$row->emergency_contact}}</td>
              <td>{{$row->gatepass}}</td>
              <!-- <td>{{$row->gatepass_rls_date}}</td> -->
              <td>{{$row->gatepass_exp_date}}</td>
						  <td>{{$row->join_date}}</td>
              <td>
              <div class="btn-group">
                ...
                <div class="dropdown-menu custom" role="menu">																																																																   
                  ...
                  <!-- <a class="dropdown-item openBtn" data-id="{{$row->id}}" data-toggle="modal" data-target="#myModal2" id="openBtn">
                    <span class="fa fa-eye" aria-hidden="true" style="color: #398439"></span> @lang('fleet.drivers_view')
                    </a> -->
                </div>
              </div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">@lang('fleet.importDepo')</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body">
        {!! Form::open(['url'=>'admin/import-drivers','method'=>'POST','files'=>true]) !!}
        <div class="form-group">
          {!! Form::label('excel',__('fleet.importDepo'),['class'=>"form-label"]) !!}
...
<div id="myModal2" class="modal fade" role="dialog" tabindex="-1">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">@lang('fleet.drivers')</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body">
...
<script type="text/javascript">
  ...
  $('.openBtn').click(function(){
    ...
    $('#myModal2 .modal-body').load('{{ url("admin/drivers/event")}}/'+id,function(result){
