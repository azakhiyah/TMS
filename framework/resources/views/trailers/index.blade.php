...
@endsection
@section("breadcrumb")
<li class="breadcrumb-item active">@lang('fleet.trailers')</li>
@endsection
  ...
  <div class="col-md-12">
    <div class="card card-info">
      <div class="card-header">
        <h3 class="card-title">@lang('fleet.manageTrailers') &nbsp; <a href="{{ route('trailers.create')}}" class="btn btn-success">@lang('fleet.addNew')</a>
        ...
        <table class="table" id="data_table" style="padding-bottom: 25px">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td>{{$row->brand}}</td>
              <td>{{$row->type}}</td>
              <td>{{$row->unit_maker}}</td>
              <td>{{$row->license_plate}}</td>
              <td>{{$row->tag_id}}</td>
              <td>{{$row->year}}</td>
              <td>{{$row->lic_exp_date}}</td>
              <td>{{$row->reg_exp_date}}</td>
              <td>{{$row->tera_sertifikat}}</td>
              <td>{{$row->tera_rls_date}}</td>
              <td>{{$row->tera_exp_date}}</td>
              <!--<td>{{$row->flowmeter}}</td>-->
              <td>{{$row->capacity}}</td>
              ...
              <td>{{ (isset($row->driver->assigned_driver)) ? $row->driver->assigned_driver->name : "" }}</td>-->
              <td>
                <div class="btn-group">
                  ...
                  <div class="dropdown-menu custom" role="menu">
                    <a class="dropdown-item" href="{{ url("admin/trailers/".$row->id."/edit") }}"> <span aria-hidden="true" class="fa fa-edit" style="color: #f0ad4e;"></span> @lang('fleet.edit')</a>
                    ...
                    <span class="fa fa-eye" aria-hidden="true" style="color: #398439"></span> @lang('fleet.view_trailers')
                    </a>
                  </div>
                </div>
                {!! Form::open(['url' => 'admin/trailers/'.$row->id,'method'=>'DELETE','class'=>'form-horizontal','id'=>'form_'.$row->id]) !!}
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">@lang('fleet.importTrailers')</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body">
        {!! Form::open(['url'=>'admin/import-trailers','method'=>'POST','files'=>true]) !!}
        <div class="form-group">
          {!! Form::label('excel',__('fleet.importTrailers'),['class'=>"form-label"]) !!}
...
<div id="bulkModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        {!! Form::open(['url'=>'admin/delete-trailers','method'=>'POST','id'=>'form_delete']) !!}
...
<div id="myModal2" class="modal fade" role="dialog" tabindex="-1">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">@lang('fleet.trailers')</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body">
...
<script type="text/javascript">
  ...
  $('.openBtn').click(function(){
    ...
    $('#myModal2 .modal-body').load('{{ url("admin/trailers/event")}}/'+id,function(result){
